...
export const getState = query({
  ...
  handler: async (ctx, args) => {
    ...
    if (
      me?.perkUsed &&
      me?.perkUsedRound === auction.currentRound &&
      me?.perk === 'SPY' &&
      currentRound
    ) {
    ...
    const completedRoundNum =
      auction.status === 'completed' ? auction.rounds.length : auction.currentRound - 1;
    ...
    if (completedRoundNum >= 1) {
      ...
      if (lastRound) {
        ...
        const winnerUserId = hostWonMain
          ? auction.host.userId
          : guestWonMain
            ? auction.guest?.userId
            : null;

...
export default function PacksPage() {
  ...
  // 10-minute rotation timer state
  const [nowMs, setNowMs] = useState<number>(Date.now());
  const TEN_MIN_MS = 10 * 60 * 1000;
  const current10MinSeed = Math.floor(nowMs / TEN_MIN_MS);
  const timeRemainingSeconds = Math.floor((TEN_MIN_MS - (nowMs % TEN_MIN_MS)) / 1000);
  ...
  // 10-Minute Random 8 Players Showcase (Leagues & Legends)
  const random8FeaturedCards = useMemo(() => {
    if (activePool.length === 0) return [];
    return shuffleWithSeed(activePool, current10MinSeed).slice(0, 8);
  ...
  return (
    <div className="space-y-8 pb-16 animate-fade-in max-w-7xl mx-auto font-sans">
      ...
      {/* ========================================================================= */}
      <section className="relative w-full rounded-3xl border border-white/10 bg-slate-950/70 p-5 sm:p-6 shadow-2xl backdrop-blur-xl overflow-hidden">
          ...
          <div className="flex flex-col items-center gap-6 py-6 animate-scale-in relative z-10">
            ...
            <div className="flex flex-col sm:flex-row items-center justify-center gap-4 sm:gap-8 py-6 perspective-[1000px] w-full max-w-3xl">
              {openedCards.map((player, idx) => {
                // Card 0 (Left): Tilted left (-rotate-6)
                // Card 1 (Center Masterpiece): Highlighted scale boost (scale-110 -translate-y-4)
                // Card 2 (Right): Tilted right (rotate-6)
        ...
        {/* EXTRATIME BRANDED PACK OPENING TRANSITION (AUTHENTIC ET CREST) */}
        {isOpening && (
          ...
          <div className="rounded-2xl border border-white/10 bg-slate-950/60 p-4 shadow-xl backdrop-blur-md space-y-4">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-white/10 pb-3">
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs font-bold px-2.5 py-0.5 rounded-full bg-lime/10 text-lime border border-lime/20 uppercase">
                  8 Players
                </span>
              </div>

              <div className="flex items-center gap-2 bg-slate-900 px-3 py-1.5 rounded-xl border border-white/10 self-start sm:self-auto">
                <Clock className="w-3.5 h-3.5 text-lime" />
                <span className="text-xs text-steel font-bold uppercase tracking-wider">Refreshes in:</span>
                <span className="font-bold text-xs text-lime font-mono">

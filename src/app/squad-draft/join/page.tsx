...
import { Loader2, RefreshCw, CheckCircle2, XCircle, Search, KeyRound, LayoutGrid } from "lucide-react";

import { randomEgyptianManagerName as randomName } from "@/lib/random-names";

export default function JoinSquadDraftPage() {
  const router = useRouter();
  const { toast } = useToast();
  const createGuest = useMutation(api.guests.mutations.create);
  const joinDraft = useMutation(api.squadDraft.mutations.join);
  ...
  const lobby = useQuery(
    api.squadDraft.queries.getLobbyByCode,
    normalizedCode.length === 6 ? { code: normalizedCode } : "skip"
  );
  const room = lobby && lobby.gameType === "SQUAD_DRAFT" ? lobby : undefined;
  ...
  const statusText = normalizedCode.length < 6
    ...
    : room === undefined
      ? "Checking room code..."
      : canJoin
        ? "Draft room found. You can join now."
        : room
          ? "This room is full or already started."
          : "No Squad Draft found with that code.";
  ...
  async function handleJoin(event: FormEvent) {
    ...
    try {
      ...
      const result = await joinDraft({ roomId: room!.roomId, guestId });
      router.push(`/squad-draft/${result.roomId}`);
    } catch (error: unknown) {
  ...
  return (
    <div className="mx-auto max-w-2xl space-y-4 animate-fade-in">
      <PageHeader
        title="Join Squad Draft"
        subtitle="Enter your rival's code and jump straight into the draft arena."
        backUrl="/"
        className="mb-3"
      />
      ...
      <form onSubmit={handleJoin} className="relative overflow-hidden rounded-2xl border border-white/10 bg-card/90 p-4 shadow-2xl backdrop-blur-xl sm:p-5">
        ...
        <div className="relative space-y-5">
          ...
          <div className={`flex items-start gap-3 rounded-xl border px-4 py-3 transition-all ${
            ...
            <div className="min-w-0">
              ...
              <p className="mt-1 text-xs font-medium leading-snug text-steel">
                You will enter the same squad draft room as soon as the code is valid.
              </p>
            </div>
          </div>
          ...
          >
            {loading ? (
              <><Loader2 className="h-4 w-4 animate-spin" /> Joining Draft...</>
            ) : (
              <><LayoutGrid className="h-4 w-4" /> Join Draft Arena</>
            )}
          </button>
        </div>
      </form>

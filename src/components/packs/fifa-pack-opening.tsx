...
export function FifaPackOpening({
  ...
  return (
    <div
      ...
      dir="ltr"
    >
      {/* ── TOP ACTION BAR (Conflict-Free on Mobile) ── */}
      <AnimatePresence>
        {!isCleanRecordMode && (
          <motion.header
          ...
          >
            {/* Left: Close / Back Button & Pack Badge */}
            <div className="flex items-center gap-1.5 sm:gap-2">
              <button
                type="button"
                onClick={handleSafeClose}
                className="flex items-center gap-1.5 rounded-full border border-white/20 bg-slate-900/90 px-3 py-1.5 text-xs font-bold text-white shadow-xl hover:border-lime/40 hover:bg-slate-800 transition-all cursor-pointer active:scale-95 shrink-0"
                title={t('packs.closeAndBack')}
              >
                <AppIcon icon={X} size={14} weight="bold" />
                <span className="text-[11px] sm:text-xs">{t('packs.closeAndBack')}</span>
              </button>
              ...
              <div className="hidden md:flex items-center gap-2 rounded-full border border-white/10 bg-slate-900/80 px-3 py-1.5 backdrop-blur-xl shadow-lg">
                ...
                <span className="font-stats text-xs font-bold tracking-wider text-white uppercase truncate max-w-[130px]">
                  {packDisplayName}
                </span>
              </div>
            </div>
            ...
            <div className="flex items-center gap-1.5 sm:gap-2">
              ...
              {stage === 'OVERVIEW' && (
                ...
                >
                  ...
                  <span className="text-[11px] sm:text-xs">{t('packs.stepByStep')}</span>
                </button>
              )}
      ...
      <div
        className="pointer-events-none absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 h-[350px] w-[350px] sm:h-[600px] sm:w-[600px] rounded-full blur-[120px] opacity-25"
        style={{ backgroundColor: tierStyle.primary }}
      />

      {/* ── CENTER STAGE CONTAINER (Responsive Padding & Max Height) ── */}
      <main className="relative z-20 my-auto flex w-full max-w-lg flex-1 flex-col items-center justify-center p-2 sm:p-4 text-center overflow-y-auto scrollbar-none max-h-[calc(100dvh-4.5rem)]">
        <AnimatePresence mode="wait">
          ...
          {stage === 'TEASE_NATION' && topCard && (
            <motion.div
              ...
              transition={{ type: 'spring', stiffness: 350, damping: 25 }}
              className="flex flex-col items-center gap-2.5 sm:gap-3.5 py-2"
            >
              <div className="text-[10px] sm:text-[10.5px] font-black tracking-[0.25em] text-steel uppercase">
                {t('packs.nation')}
              </div>

              <div
                className="relative flex h-20 w-32 sm:h-28 sm:w-46 items-center justify-center overflow-hidden rounded-2xl border-2 p-1.5 shadow-2xl backdrop-blur-2xl"
                style={{
                  borderColor: tierStyle.accent,
              ...
              <h2 className="font-display text-xl sm:text-3xl font-black tracking-wide text-white uppercase drop-shadow-md">
                {topCard.nation}
              </h2>
            </motion.div>
          )}
          ...
          {stage === 'TEASE_POSITION' && topCard && (
            ...
            >
              <div className="flex items-center gap-2">
                <CountryFlagBadge nationName={topCard.nation} className="h-3.5 w-5 sm:h-4 sm:w-6 rounded-xs" />
                <span className="text-[10px] sm:text-[10.5px] font-black tracking-[0.25em] text-steel uppercase">
                  {topCard.nation} · {t('packs.position')}
                </span>
              </div>

              <div
                className="flex h-18 w-26 sm:h-26 sm:w-34 items-center justify-center rounded-2xl sm:rounded-3xl border-2 shadow-2xl backdrop-blur-2xl"
                style={{
                  borderColor: tierStyle.accent,
              ...
              <h2 className="font-display text-lg sm:text-2xl font-black tracking-wide text-white uppercase">
                {topCard.position}
              </h2>
            </motion.div>
          )}
          ...
          {stage === 'TEASE_CLUB' && topCard && (
            ...
            >
              <div className="flex items-center gap-2">
                <CountryFlagBadge nationName={topCard.nation} className="h-3.5 w-5 sm:h-4 sm:w-6 rounded-xs" />
                <span className="text-[10px] sm:text-[10.5px] font-black tracking-widest text-lime uppercase">
                  {topCard.position.split('/')[0]} · {t('packs.club')}
                </span>
              </div>

              <div
                className="relative flex h-22 w-22 sm:h-36 sm:w-36 items-center justify-center rounded-full border-2 p-2.5 sm:p-4 shadow-2xl backdrop-blur-2xl"
                style={{
                  borderColor: tierStyle.accent,
              ...
              <h2 className="font-display text-lg sm:text-2xl font-black tracking-wide text-white uppercase drop-shadow-md">
                {topCard.club}
              </h2>
            </motion.div>
          )}
          ...
          {stage === 'WALKOUT' && topCard && (
            <motion.div
              ...
              className="flex flex-col items-center gap-2 sm:gap-3 py-1"
            >
              <motion.div
                initial={{ opacity: 0, y: -10 }}
                animate={{ opacity: 1, y: 0 }}
                transition={{ delay: 0.2 }}
                className="flex items-center gap-1.5 rounded-full border px-3 sm:px-4 py-0.5 sm:py-1 text-[11px] sm:text-xs font-black tracking-widest uppercase shadow-xl backdrop-blur-md"
                style={{
                  borderColor: tierStyle.accent,
              ...
              >
                {/* Mobile: md size fits perfectly */}
                <div className="block sm:hidden">
                  <PlayerCard player={topCard} size="md" />
                </div>
                {/* Desktop: lg size */}
              ...
              <div className="flex flex-wrap items-center justify-center gap-2 pt-0.5">
                <motion.button
                  ...
                  className="flex items-center gap-1.5 rounded-2xl bg-lime px-4 sm:px-6 py-2 sm:py-2.5 text-[11px] sm:text-xs font-black tracking-widest text-slate-950 uppercase shadow-[0_10px_25px_rgba(149,232,16,0.35)] hover:bg-lime/90 transition-all cursor-pointer active:scale-95"
                >
                  <AppIcon icon={SquaresFour} size={14} weight="bold" />
                ...
                <motion.button
                  ...
                  className="flex items-center gap-1 rounded-2xl border border-white/15 bg-white/5 px-3 sm:px-4 py-2 sm:py-2.5 text-[11px] sm:text-xs font-bold text-steel hover:text-white hover:border-white/30 transition-all cursor-pointer active:scale-95"
                >
                  <span>{t('packs.stepByStep')}</span>
                  <AppIcon icon={CaretRight} size={12} weight="bold" />
                </motion.button>
              </div>
            </motion.div>
          )}
          ...
          {stage === 'SLIDER' && currentSliderCard && (
            <motion.div
              ...
              transition={{ type: 'spring', stiffness: 350, damping: 25 }}
              className="flex w-full flex-col items-center gap-2 py-0.5"
            >
              {/* 5-Card Teaser Strip Bar (Interactive Clues Preview) */}
              <div className="flex items-center justify-center gap-1 sm:gap-1.5 pb-0.5">
                {cards.map((c, i) => {
                  ...
                  return (
                    <button
                      ...
                      className={`flex items-center gap-1 rounded-xl px-1.5 sm:px-2 py-0.5 sm:py-1 border transition-all cursor-pointer ${
                        isCurrent
              ...
              <motion.div
                ...
                animate={{ opacity: 1, y: 0 }}
                className="flex items-center gap-1.5 sm:gap-2 rounded-2xl border border-white/10 bg-slate-900/80 px-2.5 sm:px-3.5 py-0.5 sm:py-1 backdrop-blur-xl shadow-lg"
              >
                {/* 1. Nation */}
                <div className="flex items-center gap-1">
                  <CountryFlagBadge nationName={currentSliderCard.nation} className="h-3 w-4.5 sm:h-3.5 sm:w-5 rounded-xs" />
                  <span className="text-[9px] sm:text-[10px] font-bold text-white max-w-[65px] sm:max-w-[70px] truncate">
                ...
                {/* 2. Position */}
                <div
                  className="rounded-md px-1 sm:px-1.5 py-0.5 text-[9px] sm:text-[10px] font-black font-card-num border"
                  style={{
                    borderColor: tierStyle.accent,
                ...
                {/* 3. Club */}
                <div className="flex items-center gap-1">
                  <ClubCrestBadge clubName={currentSliderCard.club} className="h-3.5 w-3.5 sm:h-4 sm:w-4 rounded-full border-0 bg-transparent" />
                  <span className="text-[9px] sm:text-[10px] font-bold text-white max-w-[75px] sm:max-w-[80px] truncate">
              ...
              >
                {/* Mobile: md size fits perfectly */}
                <div className="block sm:hidden">
                  <PlayerCard player={currentSliderCard} size="md" />
                </div>
                {/* Desktop: lg size */}
                <div className="hidden sm:block">
              ...
              <div className="flex items-center justify-center gap-1.5 sm:gap-2 pt-0.5">
                ...
                <button
                  ...
                  className="flex h-8 w-8 sm:h-9 sm:w-9 items-center justify-center rounded-xl border border-white/10 bg-slate-900 text-steel hover:text-white hover:border-lime/40 disabled:opacity-30 disabled:pointer-events-none transition-all cursor-pointer"
                  title="Previous Card"
                >
                ...
                <button
                  ...
                  className={`flex h-8 sm:h-9 items-center gap-1 rounded-xl border px-2.5 sm:px-3 text-[11px] sm:text-xs font-bold transition-all cursor-pointer ${
                    isAutoPlaying
                ...
                {activeCardIndex < cards.length - 1 ? (
                  <button
                    ...
                    className="flex h-8 sm:h-9 items-center gap-1 rounded-xl bg-lime px-3 sm:px-4 text-[11px] sm:text-xs font-black tracking-wider text-slate-950 uppercase shadow-md hover:bg-lime/90 transition-all cursor-pointer"
                  >
            ...
            >
              ...
              <div className="flex flex-wrap items-center justify-center gap-2 pt-1">
                <button
                  ...
                  className="flex items-center gap-1.5 rounded-2xl bg-lime px-4 sm:px-5 py-2 sm:py-2.5 text-[11px] sm:text-xs font-black tracking-widest text-slate-950 uppercase shadow-lg hover:bg-lime/90 transition-all cursor-pointer active:scale-95"
                >
                ...
                <button
                  type="button"
                  onClick={handleSafeClose}
                  className="flex items-center gap-1.5 rounded-2xl border border-white/15 bg-white/5 px-4 sm:px-5 py-2 sm:py-2.5 text-[11px] sm:text-xs font-bold text-white hover:border-white/30 transition-all cursor-pointer active:scale-95"
                >
      ...
      {!isCleanRecordMode && (
        <footer className="relative z-20 pb-2 sm:pb-3 text-center shrink-0">
          <p className="text-[9px] sm:text-[10px] font-bold text-steel/70 tracking-widest uppercase">

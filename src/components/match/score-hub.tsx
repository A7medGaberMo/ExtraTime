    ...
    <div className="space-y-3">
      ...
      {isSimulating ? (
        <div className="flex items-center justify-center gap-2 py-1 animate-fade-in">
          ...
          <p className="text-[10px] font-black uppercase tracking-[0.25em] text-steel">
            Simulating Matchday...
          </p>
        </div>
      ) : (
...
function WinnerBanner({
  ...
  const winnerLabel =
    winnerSide === "host" ? hostName : winnerSide === "guest" ? guestName : null;

  return (
    <div
      className={`relative overflow-hidden rounded-2xl border p-4 text-center shadow-2xl backdrop-blur-xl ${viewerWon
          ? "border-lime/40 bg-gradient-to-b from-lime/10 via-card to-card shadow-[0_0_50px_rgba(149,232,16,0.15)]"
          : draw
            ? "border-amber-400/30 bg-gradient-to-b from-amber-400/10 via-card to-card"
    ...
    >
      ...
      <div className="relative space-y-1.5">
        <div
          className={`mx-auto flex h-10 w-10 items-center justify-center rounded-xl border shadow-xl ${viewerWon ? "border-lime/50 bg-slate-950 shadow-lime/15" : "border-white/10 bg-slate-950"
            }`}
        >
          {viewerWon ? (
            <Trophy className="h-5 w-5 text-lime animate-bounce" />
          ) : (
            <Swords className="h-5 w-5 text-steel" />
          )}
        </div>
        <span
          className={`inline-flex rounded-full border px-3 py-0.5 text-[9px] font-black uppercase tracking-widest ${viewerWon
        ...
        <p className="mx-auto max-w-sm text-[11px] font-medium leading-relaxed text-steel">
          {draw
  ...
  return (
    <div className="rounded-2xl border border-white/10 bg-card/95 p-4 shadow-xl backdrop-blur-xl">
      <div className="mb-3 flex items-center justify-between">
        <span className="flex items-center gap-1.5 text-[10px] font-black uppercase tracking-widest text-white">
          <Zap className="h-3.5 w-3.5 text-lime" /> Sector Matchup
        </span>
        ...
        {rows.map((row) => {
          ...
          return (
            <div key={row.key} className="flex items-center gap-2">
              <span className="w-8 shrink-0 text-[9px] font-black uppercase tracking-wider text-steel">
                {row.label}
              </span>
              <div className="flex h-3 flex-1 overflow-hidden rounded-full border border-white/5 bg-slate-950">
                <div
                  className="h-full bg-gradient-to-r from-lime/60 to-lime transition-all duration-700"
      ...
      <div className="mt-3 grid grid-cols-1 gap-2 sm:grid-cols-2">
        <div className="rounded-xl border border-lime/15 bg-lime/5 p-2 sm:p-2.5">
          <p className="text-[8px] font-black uppercase tracking-widest text-steel truncate">{hostName} Chem</p>
          <div className="flex items-baseline justify-between gap-1 mt-0.5">
            <span className="font-stats text-sm text-lime">
        ...
        <div className="rounded-xl border border-rose-500/15 bg-rose-500/5 p-2 sm:p-2.5">
          <p className="text-[8px] font-black uppercase tracking-widest text-steel truncate">{guestName} Chem</p>
          <div className="flex items-baseline justify-between gap-1 mt-0.5">
